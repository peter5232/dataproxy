...
/**
 * @author yuexie
 * @date 2024/12/4 19:13
 **/
public class TimeMilliVectorConvertor extends AbstractMultiTypeConverter<Integer>{

    public TimeMilliVectorConvertor(ValueVisitor<Integer> visitor, ValueConversionStrategy next) {
        super(visitor, next);
    }
    ...
    public void convertAndSet(ValueVector vector, int index, Object value) {
        if (vector instanceof TimeMilliVector timeMilliVector) {
            timeMilliVector.setSafe(index, this.visit(value));
        } else if (next != null){
