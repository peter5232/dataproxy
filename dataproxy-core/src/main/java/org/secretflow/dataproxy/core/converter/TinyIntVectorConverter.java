 ...
 **/
public class TinyIntVectorConverter extends AbstractMultiTypeConverter<Byte> {


    public TinyIntVectorConverter(ValueVisitor<Byte> visitor, ValueConversionStrategy next) {
        super(visitor, next);
    }
    ...
    public void convertAndSet(ValueVector vector, int index, Object value) {
        if (vector instanceof TinyIntVector tinyIntVector) {
            tinyIntVector.set(index, this.visit(value));
        } else if (next != null) {
