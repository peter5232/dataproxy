 ...
 **/
public class SmallIntVectorConverter extends AbstractMultiTypeConverter<Short> {


    public SmallIntVectorConverter(ValueVisitor<Short> visitor, ValueConversionStrategy next) {
        super(visitor, next);
    }
    ...
    public void convertAndSet(ValueVector vector, int index, Object value) {
        if (vector instanceof SmallIntVector smallIntVector) {
            smallIntVector.set(index, this.visit(value));
        } else if (next != null) {
