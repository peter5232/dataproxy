 ...
 */
@Slf4j
public class HiveRecordSender extends AbstractSender<ResultSet> {
     ...
     */
    public HiveRecordSender(int estimatedRecordCount, LinkedBlockingQueue<ResultSet> recordQueue, VectorSchemaRoot root, String tableName, DatabaseMetaData metaData, ResultSet resultSet) {
    ...
    @Override
    protected void toArrowVector(ResultSet rs, @Nonnull VectorSchemaRoot root, int takeRecordCount) {
        ...
        try {
            ...
            while (columns.next()) {
                ...
                if (filedVectorOpt.isPresent()) {
                    ...
                    recordColumnValue = rs.getObject(name);
    ...
    @Override
    protected boolean isOver(ResultSet rs) {
        try {
            if (!resultSet.next()) {
                resultSet.previous();
                return false;
